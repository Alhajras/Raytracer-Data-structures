...
\If{$current\_node \rightarrow boundingBoxIntersection(ray_p,\, ray_d,\, current\_node) = false $}
\Return $None$

\State $premitives \gets current\_node \rightarrow premitives $ 
\If{$premitives \rightarrow size \neq 0$}
...
\For{$premitive$ in $hit\_list$}
	\If{$premitive \rightarrow intersect$}
		\If{$premitive \rightarrow isClosest$}
