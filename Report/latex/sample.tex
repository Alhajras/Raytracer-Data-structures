...
\end{figure}
	...
	\end{table}
 	...
 	\begin{itemize}
 		...
 		\item  \textbf{Ray casting}: This part contains usually two for loops to go over though all the pixels in the screen which depends on the width and hight of the image to be renderd. The smaller the size of the image the faster the rendered become as the for loop gets smaller, however this is not helpful as we wnat to have a high quality image and not to loose pixels. For each pixel 
 		\item \textbf{Intersection tests}:
 		\item  \textbf{Evaluate pixel color}:
